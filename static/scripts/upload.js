/* FILE PROCESSING */
let debugImport = false;
if (!debugImport) console.log( 'Set the "debugImport" variable to true in  "static/scripts/upload.js" to enable data import debugging in the console.' );
...
var os = oboe();
os.node('timelineObjects.*', function(tlObj) {
  let placeVisit = tlObj.placeVisit;
  if ( placeVisit ) {
    let marker = new PruneCluster.Marker(
      placeVisit.location.latitudeE7 * SCALAR_E7,
      placeVisit.location.longitudeE7 * SCALAR_E7
    );
    marker.data.name = placeVisit.location.name;
    marker.data.timestamp = placeVisit.duration.startTimestampMs;
    marker.data.timestamp2 = placeVisit.duration.endTimestampMs;
    if (debugImport) console.log({marker});
...
function parseFile(file, oboeInstance) {
  ...
  var readEventHandler = function(evt) {
    if (evt.target.error == null) {
      ...
      if (debugImport) console.log({chunk});
